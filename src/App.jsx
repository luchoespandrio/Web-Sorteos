...
export default function RifasReal() {
  ...
  // Sincronizar currentUser con DB (créditos, permisos, etc.)
  useEffect(() => {
    if (currentUser) {
  ...
  const handleConfirmNumbers = (rifa, numbers) => {
    ...
    toast(isFull ? "¡Todos los números vendidos! El admin realizará el sorteo." : `¡${numbers.length} número${numbers.length>1?"s":""} reservado${numbers.length>1?"s":""}!`, "success");
    setView("lobby");
  };
  ...
  const handleRequestCredit = (amount, note) => {
    updateDB(prev => ({ ...prev, creditRequests:[...prev.creditRequests, { id:Date.now(), userId:currentUser.id, userName:currentUser.name, amount, note, date:new Date().toLocaleDateString("es-AR"), status:"pending" }] }));
  ...
  return (
    <>
      ...
      {bolilleroRifa && <BolilleroRifas rifa={bolilleroRifa} users={db.users} currentUser={currentUser} isAdmin={currentUser?.role==="admin"||currentUser?.isAdmin||false} onWinner={winner => handleRifaWinner(bolilleroRifa,winner)} onClose={() => setBolilleroRifa(null)} />}
 
      {view==="ageVerification" && <AgeVerificationScreen onVerified={() => setView("login")} onRejected={() => { document.body.innerHTML="<div style='display:flex;height:100vh;align-items:center;justify-content:center;background:#0a0a0f;color:rgba(255,255,255,.3);font-family:sans-serif;'>Acceso no permitido para menores de 18 años.</div>"; }} />}
 
      {view==="login" && <LoginScreen form={loginForm} setForm={setLoginForm} onLogin={handleLogin} error={loginError} />}
 
      {view==="lobby" && currentUser && <GameLobby currentUser={currentUser} rifas={db.rifas} onSelectRifa={r => { setSelectedRifa(r); setView("rifa-detail"); }} {...commonHeaderProps} onAdmin={() => setView("admin")} />}
 
      {view==="rifa-detail" && currentUser && liveRifa && <NumberGrid rifa={liveRifa} currentUser={currentUser} onConfirm={nums => setConfirmData({rifa:liveRifa,numbers:nums})} onBack={() => setView("lobby")} isAdmin={currentUser?.role==="admin"||currentUser?.isAdmin||false} onOpenBolillero={() => setBolilleroRifa(liveRifa)} />}
 
      {view==="profile" && currentUser && <ProfileView currentUser={currentUser} rifas={db.rifas} creditRequests={db.creditRequests} onBack={() => setView("lobby")} {...commonHeaderProps} onRequestCredit={() => setShowReqCredit(true)} />}
      ...
      {view==="admin" && currentUser && <AdminPanel db={db} setDb={updateDB} onLogout={handleLogout} onLobby={() => setView("lobby")} toast={toast} />}
 
      {view==="supervisor" && currentUser && <SupervisorPanel currentUser={currentUser} db={db} updateDB={updateDB} onLobby={() => setView("lobby")} onLogout={handleLogout} {...commonHeaderProps} />}
